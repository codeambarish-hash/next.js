...
pub struct TurboTasksBackend {
    ...
    /// `last_gc_stats_for_testing` / `last_gc_edge_loss_collected_for_testing`.
...
impl TurboTasksBackend {
    ...
    /// Tasks the most recent GC pass collected specifically via the **edge-loss** discovery buffer
    /// (a subset of `collected`) — a task already at `parent_count == 0` that a sibling cascade
    /// freed by removing its last aggregation edge. `None` if no pass has run. Test-only: this is
    /// the direct observable that the edge-loss discovery path fired, distinct from the
    /// count-zeroed path.
    #[doc(hidden)]
    pub fn last_gc_edge_loss_collected_for_testing(&self) -> Option<usize> {
        self.last_gc_stats.lock().map(|s| s.edge_loss_collected)
    }
...
impl TurboTasksBackend {
    ...
    fn snapshot_and_persist(
        ...
        let mut snapshot_phase = if self.gc_enabled {
            ...
            // `interrupted` is the signal to look for when investigating an HMR stall: it means the
            // pass wound down because an operation was blocked behind it. `abandoned` is how much
            // work that cost (re-derived by the next pass, not lost).
            gc_span.record("interrupted", stats.interrupted);
            gc_span.record("abandoned", stats.abandoned);
            gc_span.record("edge_loss_collected", stats.edge_loss_collected);
